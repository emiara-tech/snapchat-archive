...
export const useArchiveStore = defineStore("archive", () => {
	...
	const archiveCapabilities = ref<ArchiveCapabilities>({
		...EMPTY_CAPABILITIES,
	});
	...
	const totalChats = computed(() => {
		...
		return Object.values(chatHistory.value).reduce(
...
function computeIndexedMediaBytes(
	sessionIndex: ArchiveSession["index"],
): number {
	return sessionIndex.entries
		.filter((entry) => !entry.isDirectory)
		.filter(
			(entry) =>
				entry.id.path.startsWith("memories/") ||
				entry.id.path.startsWith("chat_media/"),
		)
