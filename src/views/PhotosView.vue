		...
		<div class="container">
			...
			<section class="card diagnostics-card">
				...
				<div class="diagnostics-grid">
					<span>memories_history.json: {{ archiveStore.archiveCapabilities.hasMemoriesHistoryJson ? 'found' :
						'missing' }}</span>
					<span>memories folder: {{ archiveStore.archiveCapabilities.hasMemoriesDirectory ? 'found' : 'missing'
					}}</span>
					<span>chat_media folder: {{ archiveStore.archiveCapabilities.hasChatMediaDirectory ? 'found' : 'missing'
					}}</span>
				</div>
			</section>
			...
			<div class="filters-bar">
				<div class="filter-group">
					...
					<select id="type-filter" v-model="selectedType" class="filter-select">
						<option v-for="type in types" :key="type" :value="type">{{ type === 'all' ? 'All types' : type }}
						</option>
					</select>
				</div>
				...
				<div class="filter-group">
					...
					<select id="year-filter" v-model="selectedYear" class="filter-select">
						<option v-for="year in years" :key="year" :value="year">{{ year === 'all' ? 'All years' : year }}
						</option>
					</select>
				</div>
			</div>
			...
			<div class="photos-grid" v-if="filteredPhotos.length > 0">
				<PhotoCard v-for="(photo, index) in filteredPhotos" :key="`${photo.date}-${index}`" :photo="photo"
