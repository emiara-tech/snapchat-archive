...
const analysisResults = computed(() =>
	Array.from(archiveStore.analysisResults.entries()),
);
		...
		<div class="container container-narrow">
			<header class="page-header">
				...
				<p class="page-subtitle">
					These analyzers run in the browser against parsed archive JSON. They
					are simple heuristics and expose their raw output.
				</p>
			</header>
				...
				<div v-else class="stats-grid-simple">
					<div class="stat-item">
						<span class="stat-value">{{
							(stats?.totalMemories ?? 0).toLocaleString()
						}}</span>
						<span class="stat-label">saved Memories metadata records</span>
					</div>
					<div class="stat-item">
						<span class="stat-value">{{
							(stats?.totalChats ?? 0).toLocaleString()
						}}</span>
						<span class="stat-label">chat messages</span>
					</div>
					<div class="stat-item">
						<span class="stat-value">{{
							(stats?.totalDays ?? 0).toLocaleString()
						}}</span>
						<span class="stat-label"
							>days between first and last dated record</span
						>
					</div>
					<div class="stat-item">
			...
			<section class="analysis-section">
				...
				<div v-if="analysisResults.length > 0" class="analysis-results">
					<div
						v-for="[key, value] in analysisResults"
						:key="key"
			...
			<section class="disclaimer card">
				<h3>Limits</h3>
				<p>
					The sentiment analyzer is keyword based. Activity runs are not streak
					counts. Media files are not rendered or matched to chats in this
					refactor.
				</p>
			</section>
		</div>
