		...
		<div class="container">
			<header class="page-header">
				...
				<nav class="dashboard-nav">
					<router-link to="/photos" class="btn btn-secondary"
						>Review metadata</router-link
					>
					<router-link to="/summary" class="btn btn-primary"
						>Run analysis</router-link
					>
				</nav>
			</header>
			...
			<section class="stats-section">
				<div class="stats-grid">
					<StatCard
						icon="M"
						:value="
							isLoadingStats ? '...' : formatNumber(stats?.totalMemories || 0)
						"
					...
					<StatCard
						icon="S"
						:value="
							isLoadingStats ? '...' : formatNumber(stats?.totalSnaps || 0)
						"
			...
			<section class="hero-strip card">
				<div class="hero-strip-copy">
					...
					<h2>
						{{ stats?.longestChatActiveRunDays }} days was your longest chat
						activity run, across {{ stats?.totalDays }} days of dated records.
					</h2>
					<p>
						Your parsed records stretch from
						{{ stats?.dateRange.start || "unknown" }} to
						{{ stats?.dateRange.end || "unknown" }}. Indexed Memories and chat
						media currently total {{ stats?.totalIndexedMediaSize || "0 B" }}.
					</p>
				</div>
				<router-link to="/export" class="btn btn-primary"
					>Prepare export</router-link
				>
			</section>
						...
						<div class="timeline-stats">
							...
							<div class="timeline-stat">
								<span class="stat-value">{{
									stats?.longestChatActiveRunDays
								}}</span>
								<span class="stat-label">chat activity run</span>
							</div>
							<div class="timeline-stat">
								<span class="stat-value">{{
									stats?.totalIndexedMediaSize
								}}</span>
			...
			<section class="highlights-section">
				...
				<p class="section-subtitle">
					Run local analyzers to generate inspectable, non-AI summaries of your
					archive history.
			...
			<section class="friends-section">
				...
				<div v-else-if="stats?.topFriends?.length" class="friends-grid">
					<div
						v-for="(friend, index) in stats.topFriends.slice(0, 4)"
						:key="index"
					...
					>
						...
						<div class="friend-info">
							...
							<p class="friend-stats">
								{{ formatNumber(friend.totalMessages) }} messages &middot;
				...
				<div class="export-content">
					...
					<div class="export-text">
						...
						<p class="export-description">
							Download a JSON bundle of the normalized metadata, diagnostics,
							and computed stats.
						</p>
					</div>
					<router-link to="/export" class="btn btn-primary">
